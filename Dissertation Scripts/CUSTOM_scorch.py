...
def parse_args(args):
    ...
    if os.path.isdir(params.ligand):
        ...
        params.ligands_basename = [os.path.basename(lig) for lig in params.ligand]
...
def scale_multipose_features(df):
    ...
    temp_dir = '/home/s2451611/MScProject/temp/'
    os.makedirs(temp_dir, exist_ok=True)
    file_path = os.path.join(temp_dir, f'{params.ligands_basename[0]}.csv') 
...
if __name__ == "__main__":
    # ADDED BY CIAN # logging.basicConfig(filename='feature.log', filemode='a', format='%(asctime)s - %(levelname)s - %(message)s', level=logging.INFO)
